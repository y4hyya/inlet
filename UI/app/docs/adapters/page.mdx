...
Euler's `EVK Vault eUSDC-4` on Ethereum Sepolia is an ERC 4626 vault over Circle USDC reached through the generic adapter. Monad Testnet has no Gateway, so it is a CCTP only destination, with an Inlet demo vault to deposit into. The protocol addresses behind every preset are on the [addresses page](/docs/addresses).
