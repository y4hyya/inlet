...
| `ARC_RPC`, `BASE_SEPOLIA_RPC`, `ARBITRUM_SEPOLIA_RPC`, `UNICHAIN_SEPOLIA_RPC`, `ETHEREUM_SEPOLIA_RPC` | RPC endpoints, with public defaults |
...
Destination chains and receivers come from `config/deployments.testnet.json`. The relayer serves every chain that has a receiver there: Arbitrum Sepolia, Base Sepolia and Unichain Sepolia today. It manages its nonces locally and retries nonce errors after five seconds, which matters on public RPCs that lag.
